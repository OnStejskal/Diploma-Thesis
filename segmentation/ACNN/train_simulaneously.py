...
from torch import device, set_grad_enabled, tensor
...
def accn_sim_run_training_segmentation(
    DEVICE: device,
    TRAIN_IMG_PATH: str,
    ...
    """Function that run the training of the segmentation network via the acnn framerwork, autoencoder and segmentor are trained simultaneously
    ...
    momentum = 0.999
    weight_decay = 0.001
    autoencoder_optimizer = Adam(list(encoder.parameters()) + list(decoder.parameters()), lr=lr, betas=(beta1, momentum))
    segmentor_optimizer = Adam(segmentor.parameters(), lr=lr, betas=(beta1, momentum), weight_decay=weight_decay)
    patience = 25
    min_lr = 10e-7 # minimum learning rate for scheduler
    scheduler_segmentor = ReduceLROnPlateau(segmentor_optimizer, patience=patience,min_lr=min_lr)
    scheduler_ae = ReduceLROnPlateau(autoencoder_optimizer, patience=patience,min_lr=min_lr)
    ...
    CREATE_AE_SEGMENTATIONS = True # denotes whether to plot the segmentations of the autoencoder
...
def train_model(
    ...
    Returns:
        tuple[Module, dict]: best segmentor model network and dictionary containing training and validation details
    """
    train_keys = ["train_ae_loss", "train_ae_loss_euclid", "train_ae_loss_sup", "train_seg_loss_sup", "train_seg_loss_euclid"]
    val_keys = ["val_euclid_loss", "val_ae_loss", "val_ae_loss_sup", "val_seg_loss_sup"]
    ...
    for epoch in range(num_epochs):
        ...
        for input, label in train_loader: 
            ...
            segmentation = segmentor(input).detach()
            ...
            ae_sup_loss = loss_function_ae(decoded_label, label_swapped)
            autoencoder_loss = ae_sup_loss
            autoencoder_loss.backward()
            autoencoder_optimizer.step()
            ae_euclid_loss = tensor([0]) #mse_loss(encoded_seg, encoded_label)
            train_epoch_ae_loss += autoencoder_loss.item()

            ########## SEGMENTATION TRAIN PART #############
            ...
            rs.train_step([segmentor_loss, autoencoder_loss, ae_euclid_loss, ae_sup_loss, segmentor_sup_loss, euclid_loss])
        ...
        for i, (input, label) in enumerate(val_loader):
            ...
            with set_grad_enabled(False):
                ...
                ae_sup_loss = loss_function_ae(decoded_segmentation, label)
                euclid_loss = mse_loss(encoded_segmentation, encoded_label)
                ae_loss = ae_sup_loss + lambda1*euclid_loss
                w_euclid = seg_sup_loss/euclid_loss   
                seg_loss = seg_sup_loss + w_euclid*lambda1*euclid_loss
                rs.val_step([seg_loss, euclid_loss, ae_loss, ae_sup_loss, seg_sup_loss], output, label)

                if ((epoch % 50 == 0) or (epoch == num_epochs -1)) and i < 10 and CREATE_AE_SEGMENTATIONS:
                    current_epoch_path = join(EXPERIMENT_PATH, "val_ae_segmentations", f'epoch_{epoch}')
                    makedirs(current_epoch_path, exist_ok=True)
                    plot_ae_input_output_segmentations(
                        in_segmentation= output[0].cpu().detach().numpy().argmax(0),
                        out_segmentation= decoded_segmentation[0].cpu().detach().numpy().argmax(0),
        ...
        # if the validation loss is lowest save it 
