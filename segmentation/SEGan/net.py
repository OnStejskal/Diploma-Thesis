...
# copied code from https://github.com/YuanXue1993/SegAN
...
class NetS(nn.Module):
    def __init__(self, ngpu = 1,  classes = 1, channel_dim = 3, ndf = 64):
        ...
        self.convblock1 = nn.Sequential(
            ...
            nn.Conv2d(self.channel_dim, self.ndf, 7, 2, 3, bias=False),
        ...
        self.convblock4 = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*8) x 8 x 8
        )
        self.convblock4_1 = ResidualBlock(self.ndf*8)
        self.convblock5 = nn.Sequential(
            # state size. (self.ndf*8) x 8 x 8
            nn.Conv2d(self.ndf * 8, self.ndf * 8, 5, 2, 2, bias=False),
            nn.BatchNorm2d(self.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*8) x 4 x 4
        )
        self.convblock5_1 = ResidualBlock(self.ndf*8)
        self.convblock6 = nn.Sequential(
            # state size. (self.ndf*8) x 4 x 4
            nn.Conv2d(self.ndf * 8, self.ndf * 16, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 16),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*16) x 2 x 2
        )
        self.convblock6_1 = nn.Sequential(
            # state size. (self.ndf*16) x 2 x 2
            nn.Conv2d(self.ndf * 16, self.ndf * 16, kernel_size=1, bias=False),
            nn.BatchNorm2d(self.ndf * 16),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*16) x 2 x 2
        )
        self.convblock7 = nn.Sequential(
            # state size. (self.ndf*16) x 2 x 2
            nn.Conv2d(self.ndf * 16, self.ndf * 32, 3, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 32),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*32) x 1 x 1
        )
        # self.convblock7_1 = ResidualBlock(self.ndf*32)
        self.convblock8 = nn.Sequential(
            # state size. (self.ndf*32) x 1 x 1
            nn.Conv2d(self.ndf * 32, self.ndf * 8, kernel_size=1, bias=False),
            nn.BatchNorm2d(self.ndf * 8),
            nn.LeakyReLU(0.2, inplace=True),
            # state size. (self.ndf*8) x 1 x 1
        )


        self.deconvblock1 = nn.Sequential(
            # state size. (ngf*8) x 1 x 1
            nn.ConvTranspose2d(self.ndf * 8, self.ndf * 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(self.ndf * 32),
            nn.ReLU(True),
        ...
        self.deconvblock2 = nn.Sequential(
            ...
            nn.Conv2d(self.ndf * 64 , self.ndf * 16, 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 16),
            nn.ReLU(True),
        ...
        self.deconvblock3 = nn.Sequential(
            ...
            nn.Conv2d(self.ndf * 16 * 2, self.ndf * 8, 3, 1, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 8),
            nn.ReLU(True),
        ...
        self.deconvblock4 = nn.Sequential(
            # state size. (ngf*8) x 4 x 4
            GlobalConvBlock(self.ndf*8*2, self.ndf*8, (7, 7)),
            # nn.ConvTranspose2d(self.ndf * 8 * 2, self.ndf * 8, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 8),
            nn.ReLU(True),
            # state size. (ngf*8) x 8 x 8
        )
        self.deconvblock4_1 = ResidualBlock_D(self.ndf*8)
        self.deconvblock5 = nn.Sequential(
            # state size. (ngf*8) x 8 x 8
            GlobalConvBlock(self.ndf*8*2, self.ndf*4, (7, 7)),
            # nn.ConvTranspose2d(self.ndf * 8 * 2, self.ndf * 4, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 4),
            nn.ReLU(True),
            # state size. (ngf*4) x 16 x 16
        )
        self.deconvblock5_1 = ResidualBlock_D(self.ndf*4)
        self.deconvblock6 = nn.Sequential(
            # state size. (ngf*4) x 16 x 16
            GlobalConvBlock(self.ndf*4*2, self.ndf*2, (9, 9)),
            # nn.ConvTranspose2d(self.ndf * 4 * 2, self.ndf * 2, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf * 2),
            nn.ReLU(True),
            # state size. (ngf*2) x 32 x 32
        )
        self.deconvblock6_1 = ResidualBlock_D(self.ndf*2)
        self.deconvblock7 = nn.Sequential(
            # state size. (ngf*2) x 32 x 32
            GlobalConvBlock(self.ndf*2*2, self.ndf, (9, 9)),
            # nn.ConvTranspose2d(self.ndf * 2 * 2,     self.ndf, 4, 2, 1, bias=False),
        ...
        self.deconvblock8 = nn.Sequential(
            # state size. (ngf) x 64 x 64
            GlobalConvBlock(self.ndf*2, self.ndf, (11, 11)),
            # nn.ConvTranspose2d( self.ndf * 2, self.ndf, 4, 2, 1, bias=False),
            nn.BatchNorm2d(self.ndf),
            nn.ReLU(True),
            # state size. (ngf) x 128 x 128
        )
        self.deconvblock8_1 = ResidualBlock_D(self.ndf)
        self.deconvblock9 = nn.Sequential(
            # state size. (ngf) x 128 x 128
            nn.Conv2d( self.ndf, self.classes, 5, 1, 2, bias=False),
            # state size. (self.classes) x 128 x 128
            # nn.Sigmoid()
        )
    ...
    def forward(self, input):
        ...
        if True:  #if isinstance(input.data, torch.cuda.FloatTensor) and self.ngpu is 1:
...
class NetC(nn.Module):
    def __init__(self, ngpu = 1, channel_dim = 3, ndf = 64):
        ...
        self.convblock5 = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout2d(p=0.2),
            # state size. (self.ndf*16) x 4 x 4
        )
        self.convblock5_1 = nn.Sequential(
            # input is (self.ndf*16) x 4 x 4
        ...
        self.convblock6 = nn.Sequential(
            ...
            nn.LeakyReLU(0.2, inplace=True),
            # nn.Dropout2d(p=0.2),
            # state size. (self.ndf*32) x 2 x 2
        )
